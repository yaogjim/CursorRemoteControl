...
`supervision-manager.ts` 持久化监督授权、兑换凭证摘要、暂停原因、问题级单次许可和主动检查记录。监督身份与所有者会话分离；授权使用 CDP/DOM 同次观察到的工作区身份以及明确的窗口、会话和版本范围，不从窗口标题推导。重启加载活动授权时先进入 `pending_recovery`，只能由所有者在目标和未决操作重新核对后显式恢复。

`scoped-command-service.ts` 是监督写操作的统一最终边界，核对授权、控制版本、工作区、窗口、会话、目标代际、草稿、动作、模式/模型、计划/授权版本和问题许可。`operation-journal.ts` 在派发前以强持久原子写记录内容摘要和目标，不保存正文或凭证；记录失败时不派发，重启后的不确定状态保持未知且不自动重放。`CommandExecutor` 仍负责实际 CDP 操作；所有者与 Telegram 的人工操作保持原有入口，但会先触发人工接回，暂停监督写入并取消确定未派发的监督操作。

Relay 负责按 principal（所有者或监督者）过滤 HTTP、socket 状态和命令。监督者不能切换窗口/会话、创建新会话或访问管理与调试接口；凭证在处理中失效时命令安全拒绝，不回退到所有者路径。网页中的监督面板只反映这些服务端记录和回执，断线或身份变化时清空旧监督上下文。
