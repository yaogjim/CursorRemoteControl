...
### 4.2.2 托管首批实施：诊断与复验边界

以项目内 `工作区_agent_托管方案_622ce6c4.plan.md` 的托管目标为主线，历史候选验收、当前安装实例验收、源码测试与构建结果分别记录。上节的候选成功样本保留为历史记录，不代表后续安装实例、所有计划卡或整个托管功能已通过。

本批计划发现及全文读取失败保留原 `error`，新增可选 `failure`：稳定失败码、失败阶段、`commandId` 与调用方请求的窗口/会话/计划标识。诊断标识有界，不回传其他目标、文件路径或凭证；目标变化后切回不能消除已观察到的失效。Plans 栏显示当前目标、结果目标和可解释失败；`reachedStart` 只说明“到当前滚动容器顶部”，不能证明全部历史已加载，结果继续标记 `partial`。计划详情明确单次文件快照不是完整任务或会话，目标、约束、验收条件和关键决策未结构化取得时标为未知，不从摘要猜测。

2026-09-15 首批运行基线：3000 端口的进程路径为已安装 0.2.0 的 `dist/server/bundle.mjs`；通过原始网页可见 Plans 入口打开本次工作会话的执行摘要卡，页面拒绝全文读取并标明仅为摘要。只读 DOM 探测在同一窗口 `2507EEB64DBBEDBDA8F8CD89005BD9BE`、会话 `54404e47-d580-4fcf-875c-a34a810c4548` 观察到 `plan-execution-label`（Build Plan in Parallel），没有关联的 `toolCallId`。这只确认该摘要卡缺少可直接核对的工具身份，不代表其他创建计划记录不存在或历史发现选择器失配。对无明确文件引用且无工具调用标识的计划，候选代码在文件定位阶段明确拒绝，不猜文件名、不读取磁盘。

当前安装实例尚未加载本批源码修改；正式全文入口验收点 1 保持待复验通过。工作区 URI 已在本机单根工作区只读探测到，多根和远程工作区归属仍待验证。受限身份、定向托管执行、跨重启操作安全、问题决策和监督恢复未包含在本批实现中。

Plans 展开区另列连接、提取、快照记录的最近成功观察时间、界面运行状态及活动来源。提取等待或未启动不标成失败；连接变化同步重绘，已有观察超过 15 秒后显示依据过期。`lastExtractionAt` 不随普通状态补丁持续广播，故这里是网页最近取得的快照依据，不是实时提取心跳；过期不代表服务端停止提取，Idle 不代表完成。任务完成仍为未知，监督者主动检查未启用。本批没有增加轮询或改变状态传输协议。

2026-09-15 收尾验证：最新源码 `npm test` 为 682 项通过、0 失败，退出码 0；`npx tsc --noEmit`、`node --check src/client/app.js`、`npm run build`、`npm run build:ext` 和 `git diff --check` 均成功。隔离候选 `127.0.0.1:3001` 的实例标识为 `449c3fef0626b5d08d73ec95c166f166`，健康接口报告的服务器 bundle 摘要 `a992ff398bfb8788966bbbe0e622c7c55ad96353ea91575df4607c3364c001cc` 与最终构建文件一致；该摘要不覆盖客户端资源。另经浏览器读取确认新版状态依据实际可见，窗口/会话标识与健康接口一致，页面依据过期时接口仍报告提取正常，边界说明可见。本次浏览器首轮等待新鲜状态且未见目标摘要卡，后续只读核对确认状态依据正常展示，但没有重做全文读取；最新候选的全文、失败详情及任务依据弹窗端到端复验仍未完成，不以源码测试或历史成功样本替代。

候选验证结束后，已核对实例标识及进程路径再停止候选进程 47620；随后确认 3001 无监听，正式 3000 仍由原进程 67204 监听且健康接口返回 HTTP 200。浏览器已解锁；本批未安装扩展或替换正式服务。

### 4.2.3 托管阶段 2—5 源码状态

后续源码已经加入独立、可过期和可撤销的监督身份。授权记录绑定稳定工作区身份、可读/可写会话、目标与验收说明、计划/授权/控制版本、期限、操作次数以及允许的动作、模式和模型。监督 socket 只接收授权范围内状态；工作区或会话不匹配时返回最小空快照。导航、切换窗口、新会话及敏感管理接口保持所有者边界，监督凭证失效后不会回退为所有者命令。

监督写入由 `ScopedCommandService` 统一复核，并由 `OperationJournal` 在派发前强持久记录。最终派发前再次核对授权、控制版本、同次观察的工作区/窗口/会话/目标代际、草稿、动作、内容摘要及问题级单次许可。持久化失败不派发；重启恢复出的待派发、派发中或已派发未确认记录转为未知并阻止自动重放。人工操作优先：工作区观测新鲜时暂停对应授权，观测过期时保守暂停全部活动授权，并取消确定未派发的监督操作。所有者恢复只清除 `human_takeover`，不清除待决问题、监督失联或未知结果等其他暂停原因。

现有网页增加托管目标、版本、授权范围、暂停原因、问题、主动检查和操作回执，并在断线、身份变化或监督上下文未确认时清除旧上下文并锁定写入口。问题决定绑定当前可执行动作、目标与版本，只能消费一次。检查记录和逾期暂停、重启后的 `pending_recovery` 及所有者显式恢复已经持久化并接入 Relay/Web。Telegram 仍按既有所有者通道处理；在没有明确通知接收人与受限身份授权前，不把它记为监督通知或决策通道。

2026-09-15 当前工作树回归：`npm test` 779 项通过、0 失败；`npx tsc --noEmit`、`node --check src/client/app.js`、`npm run build`、`npm run build:ext` 和 `git diff --check` 成功。以上是源码、模拟测试和构建证据；未安装到正式 3000，也未执行真实 Cursor 写入、真实通知或故障演练。正式计划全文、获准低风险操作、通知接收与回复、跨进程失联/重启及最终人工验收仍需明确参数和授权。

同日再次通过正式 `3000` 的正常网页入口做一次有界只读复验：健康状态连接正常且目标窗口为 `2507EEB64DBBEDBDA8F8CD89005BD9BE`，但正式版本不暴露可与 `54404e47-d580-4fcf-875c-a34a810c4548` 精确比对的会话 UUID。页面唯一计划卡仍是执行摘要；View Plan 明确显示“以下仅为会话摘要”，没有文件名、内容版本或全文。仅一次历史计划发现以“会话滚动位置已改变”（`scroll_changed`）失败后立即停止，没有重试或绕过。正式实例仍是旧部署，未加载当前工作树的结构化失败诊断；本次没有发送消息、审批、切换窗口/会话或创建新会话。验收点 1 继续记为未通过，而不是用标题或摘要猜测计划关联。

用户随后授权发送一条只创建计划关联、不执行实现的精确提示。页面确认文本逐字一致并已进入当前 Cursor 会话的 `1 queued` 队列；由于当前会话仍在执行本次工作，90 秒内未形成新的 human 回合、CreatePlan 卡或文件关联，且没有审批或其他副作用。为避免在同一活动会话叠加第二个排队写入，已授权的 `ACK-SUPERVISION-0.2.0` 低风险测试消息没有继续发送。Telegram 接收人授权由用户取消，因此未发送通知。

隔离恢复演练使用最新 `dist/server/bundle.mjs`（SHA-256 `aca2f1009fb4ecd7491f36cdc52ab52c50eae0431016a8be7993443be4e81502`）绑定 `127.0.0.1:3001`。只读授权绑定目标窗口和会话、写范围为空、操作上限为 0；首次实例 `42b058a7eecedac29ca084eb9b901f7c` 停止后，以同一数据目录启动实例 `75f3fa0279477d42ca118ceeec9a4376`，授权进入 `pending_recovery`，再由所有者显式恢复为 active。最后撤销授权并停止候选，确认 3001 无监听，正式 3000 仍由 PID 92427 监听。演练没有向 Cursor 发送消息或审批。
